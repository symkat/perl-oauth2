...
also are likely to have had to choose the exact variant of the protocol that
the I<service provider> should assume you are using, which is called a
I<flow>.

You will also need information about the I<service provider>.  Specifically
you will need to have an I<Authorization Endpoint> and a I<Token Endpoint>.
They likely will have important documentation that you will want to read,
including documentation for the APIs that you want to be able to access.
...
I<flow>s you will need to detect that and do the handshake yourself.  You
get your choice from L<LWP::Authen::OAuth2> of detecting it through a
callback, or by trapping an error.

Also even if you are in a I<flow> that supports it, retry success is not
guaranteed.  The I<user> may revoke your access, the I<service provider> may
decide you are a suspicious character, there may have been a service outage,
etc.  L<LWP::Authen::OAuth2> will let you know if this happened, and it is
your decision how to deal with it.
...
pretty close to the same thing.  But, in fact, a single I<consumer> may
actually have multiple I<client>s.
...
It is, of course, up to the I<service provider> what I<flow>s are available
and what they are called.  Here is a list of common ones from
L<http://tools.ietf.org/html/rfc6749#section-2.1|RFC 6749>.
...
for a Perl I<consumer>.

=item user-agent-based application

JavaScript application running in a browser that wants to make AJAX calls.
Can't keep secrets.  A Perl I<consumer> should not choose this.

=item native application

Application installed on a I<user>'s machine.  Can't keep secrets.  Possibly
appropriate for a Perl I<consumer>.  It is not impossible that for some
applications and I<service provider>s this will be the most convenient choice
even if it is not a perfect fit.

=back

Just to give a sense of variability, as of the time of this writing, Google
documents no less than six I<flow>s at
L<https://developers.google.com/accounts/docs/OAuth2>, none of which have
been given the above names.  They rename the top two, split
I<native application> into two based on whether your application controls a
browser, and add two new ones.  Half require interactions that are not
described in the specification, and the rest have added optional arguments.
...
is, and what I<flow> to use.  Being public, it can be shared
...
the OAuth 2 specification.  It is required in some I<flow>s by the
I<service provider>.
...
could usefully use any value other than C<code>.  However there are flows
which I<service provider>s can support which do other things.  For example
the I<user-agent-based application> I<flow> will use a I<response_type> of
I<token>.
...
actually mandate that they be accepted.  However they often are.  And,
of course, a given service provider can add more parameters at will, and
require (or not) different things by flow.
...
passed as a get parameter.  The web server would then pick these up, finish the handshake, and then redirect the user elsewhere.
...
specification includes more that might prove useful depending on your I<flow>
and I<service provider>.  However programmers using L<LWP::Authen::OAuth2>
are only likely to directly encounter them when subclassing
L<LWP::Authen::OAuth2::ServiceProvider>, and hopefully will discover the
applicability of those I<grant_type>s from the I<service provider>'s
documentation.
...
could encounter.  In practice I<service provider>s can add more, and it is
not guaranteed that they will correctly implement 
...
though the specification recommends against using a get parameter.

This is the most common type of I<access_token>.  However depending on the
I<service provider> and I<flow> you could be handed another.

Once you have completed your handshake, L<LWP::Authen::OAuth2> will
automatically sign any C<get>, C<post>, or C<put> for you if it understands
the token type.  If you run across a token type that is not already in the
distribution (or is in the distribution, but your I<service provider>'s
version has "quirks") see
L<LWP::Authen::OAuth2::AccessToken> for advice on how to add support.
...
and if unlucky could get an error as above.
...
But note that not all flows offer a refresh token or the possibility of this
request.  If you're on one of those flows then you need to send the I<user>
back to the I<service provider> for authorization renewal.  From the
I<user>'s point of view this is likely to be painless because it will be done
with transparent redirects.  But there is no hiding the need for this from
the I<consumer>.
