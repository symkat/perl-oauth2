...
C<authorization_code>.  (Which in every other interaction is simply called
the I<code>.  Lovely.  Required, useless, AND inconsistently named.)

=item Access Token Request

Once the I<consumer> has a I<code> the consumer can submit an
I<Access Token Request> by sending a get (or post) request to the
I<Token Endpoint> with the I<grant_type>, I<code>
