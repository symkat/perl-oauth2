...
L<http://tools.ietf.org/html/draft-ietf-oauth-v2-31#section-4.1.2.1|a draft>
for a list of the possible errors.  The draft indicates that optional fields
could be included with extra detail.  I would not advise optimism on that
point.
...
L<http://tools.ietf.org/html/draft-ietf-oauth-v2-31>.
...
In most cases this field seems redundant.  But it is in the specification,
and is actually required by I<service provider>s, so there is no point in
...
failure depend on the phase of the Moon, but the others are real examples.)

The C<request_tokens> method of L<LWP::Authen::OAuth2> will make this
request for you, read the JSON and create the tokens.  It is up to you to
be notified by callback or direct request for the values of the tokens so
that you can store them, then retrieve them to skip the handshake on
future API calls.
...
is automatically authorized until that token expires.  This is the most
common type of I<access_token> but, depending on the I<service provider> and
your I<flow>, you may be handed another type of token that has to be signed
in a different way.
...
the token type.  If you run across a token type that is not understood, see
L<LWP::Authen::OAuth2::AccessToken> for advice on how to add support for it.
...
C<< <cascio@helminthist.net> >> for helpful feedback on early drafts.
